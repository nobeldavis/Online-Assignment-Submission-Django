...
from .models import Teacher_login_informa, Student_login_informatn,Student_courses_with_Teacher_name, Grade_Student, Teacher_Assignment_upload_File, Student_Submit_Assignment_pro, Assignment_Comments
...
def add_assignment_up_page(request):
    if request.method=="POST":
        ...
        print(ass_name, course_ass_up, date_ass_up, due_date_ass_up, title_ass_up,  details_ass_up)

        Teacher_Assignment_upload_file_data = Teacher_Assignment_upload_File(Assignment_name=ass_name, course=course_ass_up, date=date_ass_up, due_date=due_date_ass_up, title=title_ass_up, Details=details_ass_up, Out_Of_Grade=grade_ass_up)
...
def submit_assignment_student(request):
    if request.method == "POST":
        ...
        Student_Submit_Assignment_pro_data = Student_Submit_Assignment_pro(attachment =url_file1, course=cor_name_stu_submit1, Student_ID=stu_id_stu_submit1, Assignment_name=ass_name_stu_submit1, date_now=None)
...
def deleteAssignments(request):
    if request.method == "POST": 
        ...
        for n in range(100):
            Assignment_delete = Student_Submit_Assignment_pro(id=n)
            Assignment_delete.delete()
    
    return redirect(f'stu_course')

def deleteAssignments2(request):
    if request.method == "POST": 
        """ assign_id = request.POST.get('assign_id') """
    
        for n in range(100):
            Assignment_delete = Teacher_Assignment_upload_File(id=n)
            Assignment_delete.delete()
